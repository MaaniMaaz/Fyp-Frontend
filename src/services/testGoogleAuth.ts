// Temporary test authentication to bypass Google Identity Services issues
class TestGoogleAuth {
  ...
  async authenticateWithGoogle(): Promise<{ success: boolean; user?: any; error?: string; isNewUser?: boolean }> {
    try {
      ...
      // Send to backend with test token
      const response = await fetch(`${this.baseURL}/auth/google`, {
        ...
        body: JSON.stringify({ 
          googleToken: 'test-google-token-' + Date.now(),
          user: userData 
        }),
      });
...
const testGoogleAuth = new TestGoogleAuth();
